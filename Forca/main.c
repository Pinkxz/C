...
int main() {
    ...
    while (tentativas > 0 && acertos < tamanho) {
        ...
        if (!letraValida) {
            // Verifica se a letra digitada est� presente na palavra
            for (i = 0; i < tamanho; i++) {
    ...
    if (acertos == tamanho) {
        printf("\nParab�ns! Voce acertou a palavra: %s\n", palavra);
    } else {
